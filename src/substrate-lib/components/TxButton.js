...
function TxButton({
  ...
  const queryResHandler = result => {
    const tester = result.get('tester').toHuman()
    const res = Object.assign(JSON.parse(result.toString()), { tester })
    return result.isNone ? setStatus('None') : setStatus(JSON.stringify(res))
  }
  const query = async () => {
    const transformed = transformParams(paramFields, inputParams)
